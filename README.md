...
4. Run cov2bootstrap.R script. This script takes in boot table and randomly samples rows from it as genomes (number of genomes is set with --Nsub parameter, default is sampling the same number of rows as the boot table has). Then, it calculates number of connections between regions (as presented in the 2nd column of boot table) and writes the results in table format for each iteration and in long format after it finishes. If boot_tbl is not passed as a parameter, connection is inferred ONLY from identical genomes.
5. Run cov2aggr.R script. It takes in the long table output from cov2bootstrap and calculated average values for RRs over replications and writes the result as a symmetrical log-transformed table. It also calculates star matrix (cells marked with star means that the range of values of this cell does not cross 1 over all of the replications). 
6. visualization is done with cov2heatmaps.R and cov2mds.R
